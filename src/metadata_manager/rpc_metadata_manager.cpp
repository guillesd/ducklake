...
RpcMetadataManager::RpcMetadataManager(DuckLakeTransaction &transaction) : DuckLakeMetadataManager(transaction) {
}

unique_ptr<QueryResult> RpcMetadataManager::ExecuteQuery(DuckLakeSnapshot snapshot, string &query) {
	...
	return connection.Query(StringUtil::Format("FROM rpc_call_by_name(%s, %s)", catalog_literal, SQLString(query)));
}

unique_ptr<QueryResult> RpcMetadataManager::Execute(DuckLakeSnapshot snapshot, string &query) {
	return ExecuteQuery(snapshot, query);
}

unique_ptr<QueryResult> RpcMetadataManager::Query(DuckLakeSnapshot snapshot, string &query) {
	return ExecuteQuery(snapshot, query);
}

string RpcMetadataManager::GetLatestSnapshotQuery() const {
	return R"(
	SELECT * FROM rpc_call_by_name({METADATA_CATALOG_NAME_LITERAL},
